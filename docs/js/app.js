...
class FinanceApp {
    ...
    addThemeToggleListeners() {
        ...
        // Отслеживание ввода пароля для индикатора надежности
        ...
        if (regPassword2) {
            regPassword2.addEventListener('input', () => this.checkPasswordMatch());
        }
    ...
    showPasswordField() {
        ...
        const securityInfo = document.getElementById('loginSecurityInfo');
    ...
    async handleLogin(event) {
        ...
        try {
            ...
            if (foundUser) {
                // Если у пользователя есть соль, используем хеширование
                if (foundUser.salt) {
                    const hashedPassword = await this.hashPassword(password, foundUser.salt);
                    if (hashedPassword === foundUser.password) {
                        user = foundUser;
                    }
                } else {
                    // Обратная совместимость для старых паролей без соли
                    if (foundUser.password === password) {
                        user = foundUser;
                        // Обновляем пароль с солью для безопасности
                        const salt = this.generateSalt();
                        const newHashedPassword = await this.hashPassword(password, salt);
    ...
    // Хеширование пароля с солью используя Web Crypto API
    ...
    checkPasswordStrength(password) {
        ...
        return {
            ...
            feedback: feedback,
            text: ['Очень слабый', 'Слабый', 'Средний', 'Хороший', 'Сильный', 'Очень сильный'][Math.min(strength, 5)]
        };
    }
        ...
        strengthIndicator.innerHTML = `
            <div style="margin-top: 8px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="font-size: 0.85em; color: var(--text-light);">Надежность пароля:</span>
                    <span style="font-size: 0.85em; color: ${colors[strength.score]}; font-weight: 600;">${strength.text}</span>
                </div>
                <div style="background: var(--border); height: 6px; border-radius: 3px; overflow: hidden;">
                    <div style="background: ${colors[strength.score]}; height: 100%; width: ${widthPercent}%; transition: width 0.3s ease;"></div>
                </div>
                ${strength.feedback.length > 0 ? `
                    <div style="margin-top: 4px; font-size: 0.75em; color: var(--text-light);">
                        Рекомендации: ${strength.feedback.join(', ')}
                    </div>
                ` : ''}
            </div>
        `;
    ...
    checkPasswordMatch() {
        ...
        if (password === password2) {
            matchIndicator.innerHTML = `
                <div style="margin-top: 8px; color: var(--success); font-size: 0.85em; display: flex; align-items: center; gap: 4px;">
                    <span>✓</span> Пароли совпадают
                </div>
            `;
        } else {
            matchIndicator.innerHTML = `
                <div style="margin-top: 8px; color: var(--danger); font-size: 0.85em; display: flex; align-items: center; gap: 4px;">
    ...
    async handleRegister(event) {
        ...
        const strength = this.checkPasswordStrength(password);
        if (strength.score < 3) {
            alert('❌ Слишком слабый пароль! Пожалуйста, используйте более надежный пароль.');
            return;
        }
    ...
    async registerUserLocally(name, email, password) {
        const users = JSON.parse(localStorage.getItem('users')) || [];
